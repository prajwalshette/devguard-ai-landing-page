...
export const generateSecurityReportPDF = async (data: SecurityReportData): Promise<PDFResult> => {
  const doc = new jsPDF();
  ...
  if (resolutionChartImg) {
    ...
    const statsYPos = yPos + 15;
    
    doc.setTextColor(50);
    doc.text(`Total Found: ${totalFound}`, statsX, statsYPos);
    doc.text(`Total Resolved: ${totalResolved}`, statsX, statsYPos + 8);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(34, 197, 94);
    doc.text(`Resolution Rate: ${resolutionRate}%`, statsX, statsYPos + 16);
...
export const exportSecurityReportPDF = async (data: SecurityReportData) => {
  const result = await generateSecurityReportPDF(data);
  result.download();
};
